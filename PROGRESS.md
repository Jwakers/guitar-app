...
- [x] **Coaching notes + purpose layout** — Move purpose (and short description if needed) above the tab. Make coaching notes sticky on desktop (side panel); stacked but prominent on mobile. Keep practice and `/drills/[id]` consistent via shared detail sections.
...
- [x] **Session progress “Exercise x of y”** — Shown in the sticky practice dock (with metronome + actions) so it stays visible while scrolling; top header slimmed to Exit / replay context.
